...
begin
  ...
  begin
    ...
    try
      CKRUN.CHECKED := Inifile.ReadBool('YxDServer', 'Auto', False);
      CKAUTORUN.CHECKED := Inifile.ReadBool('YxDServer', 'AutoRun', False);
      CKREBOOT.CHECKED := Inifile.ReadBool('YxDServer', 'ReBoot', False);
      EdtReBootT.text := Inifile.ReadString('YxDServer', 'ReBootT', '');
      CKDEBUG.CHECKED := Inifile.ReadBool('YxDServer', 'DEBUG', False);
      if Inifile.ReadBool('YxDServer', 'HttpType', False) then
        RbHTTP.CHECKED := True
      else
        RbWEB.CHECKED := True;
      Inifile.ReadString('YxDServer', 'ReBootT', '');
      EdtWorkCount.text := Inifile.ReadString('YxDServer', 'Pools', '32');
      EdtPort.Text := Inifile.ReadString('YxDServer', 'Port', '8080');
      EdtSize.Text := IntToStr(Inifile.ReadInteger('YxDServer','LogSize',10));
    finally
      FreeAndNil(Inifile);
    end;
  end;
